...
public class HttpDownloader {
	...
	public static String download(String fileUri, String fileType) {
		...
		String result = ServiceConfig.ACTION_FAILED;
		InputStream inputStream = null;
		HttpURLConnection conn = null;
		try {

			Log.e("YDINFOR::",
					"-----------------------HttpURLConnection创建链接----------------------");
			...
			if (contentLength <= MUTI_THREAD_SIZE_POINT) {
				...
				if (fileUtils.isFileExist(fileName)) {
					...
					long fileLength = fileUtils.writeToSDCard(fileName,
							inputStream);
					// 如果fileResult=null,下载失败。
					if (contentLength == fileLength) {
						result = ServiceConfig.ACTION_SUCCESS;
					} else {
						// 文件不完整，删除
						String removeFile = fileUtils.getLocalFileDir()
								+ fileName;
			...
			result = ServiceConfig.ACTION_FAILED;
			e.printStackTrace();
		} finally {
			try {
				if (inputStream != null) {
